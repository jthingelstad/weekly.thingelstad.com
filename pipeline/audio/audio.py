...
# Strip whichever generated-notice prefix the archive file carries — current
# (from data/issues) plus the two legacy variants in git history. parse_archive_file
# in pipeline/content/content.py uses the same tolerance set.
GENERATED_NOTICE_RE = re.compile(
    r"^<!-- Generated by (?:scripts/content|pipeline/content/content)\.py "
    r"from data/(?:issues|buttondown); do not edit directly\. -->\r?\n"
)
...
def _body_text(issue: str) -> str:
    """Return the canonical editorial body for an issue (no front matter).

    Used by ``scripts review`` to LLM-check the synthesized script against the
    source body. The canonical body lives in ``data/issues/{N}/archive.md``;
    we strip its YAML front matter to get just the markdown."""
    path = ISSUES_ROOT / issue / "archive.md"
    if not path.exists():
        raise RuntimeError(f"Canonical body not found: {path}")
