...
(`/srv/www/wordpress/logs/$host-access.log`).
...
zgrep is just grep that handles both normal and gzipped files. Pipe that
into cut to pull out just the data we want. The two sed commands pull
out data that would mess up the aggregation (the IP address of the
requester and part of the filename). Sort puts prepares the stream for
uniq to do the counting. Then do a numeric sort in reverse and show the
top 20 404's in all log files.
...
As an aside, sort combined with `uniq -c` has to be one of the most
