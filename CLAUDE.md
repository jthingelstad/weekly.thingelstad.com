...
The workshop_bot (`apps/workshop_bot/`) was rebuilt around a **jobs spine**: every user-facing action is a deterministic Python job in `apps/workshop_bot/jobs/`, fired by the `/workshop …` slash surface (host: Eddy; `manage_guild`-gated) and/or by cron. The slash surface is grouped by content artifact — `/workshop issue {start,update,status,final,haiku,subject,cta,publish}`, `/workshop links scan`, `/workshop promo {prep,metrics}`, `/workshop campaign {add,report,copy,sunset}`, `/workshop goal {set,done}`, plus a top-level `/workshop status`. (The internal job names — `update-draft`, `create-final`, `compose-meta`, `pinboard-scan`, `daily-metrics`, `goal-achieved`, … — are dash-cased; the slash names are the operator-facing aliases. The map lives in `apps/workshop_bot/CLAUDE.md`.) Some jobs make small encapsulated LLM calls; most are pure Python. Single-asset locking (`job_locks` table) serializes jobs that write the same workspace file.
