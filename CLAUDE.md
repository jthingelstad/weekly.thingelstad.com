...
Issue-assembly flow: `start-issue` → `update-draft` (daily 17:00 CT; pure projection of Pinboard/micro.blog/asset files into `draft.md`; Eddy reviews Tue–Fri) → `create-final` (Eddy reorder review → `final.md`) → `compose-haiku` / `compose-meta` / `compose-cta` (run on demand, any order — Eddy/Patty post options to a channel, Jamie reacts to pick) → `build-publish` (assembles `publish.md`; refuses with a missing-list until the required assets exist) → `pipeline/content/content.py publish --issue N` creates the Buttondown draft. Parallel tracks: `pinboard-scan` (Linky, Mon–Fri 06:30/18:30 during the window), `promotion-prep` (Marky, RSS-triggered post-ship), `daily-metrics` (Marky, daily 19:00 CT) + `add-campaign` / `campaign-report`. Issue assets are standalone files in `s3://files.thingelstad.com/weekly-thing/{N}/` (`intro.md`, `currently.md`, `haiku.md`, `metadata.json`, `cta-*.md`, `draft.md`, `final.md`, `publish.md`).
