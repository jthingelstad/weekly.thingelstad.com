...
the in-flight issue's *content* completeness, use ``/workshop job
issue-status``; for campaign performance vs. expectation, use
``/workshop job campaign-report``.
...
async def run(ctx: "_base.JobContext") -> "_base.JobResult":
    ...
    if w is None:
        lines.append("• issue window: *(none — `/workshop job start-issue` to set one)*")
    else:
        n = int(w["issue_number"])
        lines.append(
            f"• issue window: **WT{n}** · pub {w['pub_date']} ({_rel(w['pub_date'])}) · "
            f"cutoff {w['end_date']} · {w.get('day_count', 7)}-day "
            f"— `/workshop job issue-status` for content state"
        )

    g = db.get_active_goal()
    if g is None:
        lines.append("• goal: *(none — `/workshop job set-goal <kind> <value>`)*")
    else:
        lines.append(
