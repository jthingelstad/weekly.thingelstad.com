#!/usr/bin/env python3
"""Backup workshop.db with compression and tiered retention pruning.
...
log = logging.getLogger("workshop_backup")

_SCRIPT_DIR = Path(__file__).resolve().parent
_WORKSHOP_DIR = _SCRIPT_DIR.parent
_DEFAULT_DB = _WORKSHOP_DIR / "data" / "workshop.db"
_DEFAULT_BACKUP_DIR = Path.home() / "workshop-backups"

_FILENAME_RE = re.compile(r"^workshop-(\d{4}-\d{2}-\d{2}-\d{6})\.db\.gz$")
...
def _backup_dir() -> Path:
    return Path(os.getenv("WORKSHOP_BACKUP_DIR", str(_DEFAULT_BACKUP_DIR)))


def _db_path() -> Path:
    return Path(os.getenv("WORKSHOP_DB_PATH", str(_DEFAULT_DB)))
...
def create_backup(db_path: Path | None = None, backup_dir: Path | None = None) -> dict:
    ...
    filename = f"workshop-{now.strftime(_TIMESTAMP_FMT)}.db.gz"
