#!/usr/bin/env python3
"""Clean local development cruft under apps/workshop_bot/.

By default this removes transient cache directories only. Use --db to also
remove the local SQLite at apps/workshop_bot/data/workshop.db.
"""
...
def main() -> None:
    ...
    for path in WORKSHOP_DIR.rglob("*"):
        ...
        if path.name in CACHE_DIR_NAMES and _remove_path(path):
            removed.append(str(path.relative_to(WORKSHOP_DIR)))
