...
| `compose-meta` | manual | Two reaction picks in `#editorial`: (1) Eddy returns **5 `WT<N> — <theme>` subject options** (`prompts/eddy/compose-subject.md`, returned as a numbered list — *that exact prompt, verbatim*) → Jamie reacts 1️⃣–5️⃣; (2) Eddy returns 3 description options (`prompts/eddy/compose-description.md`) → Jamie picks. Writes `metadata.json` (subject/description generated; image/slug/number/publish_date deterministic). Skipping the description pick still writes `metadata.json` with an empty description. Reads `data/buttondown/emails/*.json` for recent subjects (fed to the *description* step). Required for ship. |
