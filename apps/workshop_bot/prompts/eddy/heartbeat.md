...
Call `issue__current_window` first. If it returns `{error: ...}` (no active window), return exactly `PASS` and stop — there's nothing to do between issue cycles. Also `PASS` and stop if today's date is **before** the window's `start_date` or **after** its `pub_date`: outside the active cycle there is no draft to review and no editorial signal to surface.
...
- If Jamie's started drafting (`workspace__list_files(N)` shows a `draft.md`), give it a quick read. You're looking for early-draft signals, not a full edit.
