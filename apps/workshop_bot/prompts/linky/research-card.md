...
The job hands you one of three source types in the `## The link` block:
...
The URL field is always the article URL — the one we'd bookmark. (For `lobsters` items, the discussion URL is a *secondary* link surfaced separately.)
...
4. **Decide.**
   - For **`popular`** and **`lobsters`** items the bar is **interesting to Jamie**, not "fits the Weekly Thing" — he decides what to bookmark. For `lobsters`, the score / comment count and tags are signal but not a substitute for actually reading the thing; a 200-point post on a topic Jamie doesn't engage with should still be skipped. If it doesn't clear that bar, respond EXACTLY: `SKIP: <one-line reason>` (no card). The job records your verdict and won't surface this URL again.
...
- **`💬` action line:**
  ...
  - `lobsters`: `_React ✅ / 👍 to bookmark (toread + public, blank description), or reply to bookmark with your reply as the description._`
...
- Two failure signals (case-sensitive, must be the entire first line):
  - `SKIP: <reason>` — applies to `popular` and `lobsters`; tells the job "not interesting enough, never surface again."
