...
> Draft — refine in your voice. Mechanics are accurate to current rendering; the *why* is yours to
> sharpen.
...
| 9 | Echoes | `## Echoes` | `closer.md` (written by Thingy) — see [`echoes.md`](echoes.md) | optional (SKIP allowed) |

**Featured** posts splice in around Notable/Journal/Briefly — see
[`featured-posts.md`](featured-posts.md). The trailer after Echoes is the haiku → "discuss on
Reddit" line → `👨‍💻`.

## Per-section formatting

- **Notable** — leads with the Reddit-tag line (`_You can discuss any of these links at the Weekly
...
- **Journal** — status update: `[Weekday @ H:MM AM/PM](url)` + content. Titled post (elevated):
  `### [Title](url)  \n{Weekday @ H:MM AM/PM}` + content. Weekday only (no date — the whole issue
...
Section names aren't uniform across the back catalog (link extraction accepts the variants):
