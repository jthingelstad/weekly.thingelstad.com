...
The generated files in `src/archive/` are not the editing surface. Archive cleanup should happen in `data/buttondown/bodies/` and the metadata snapshots in `data/buttondown/emails/`, then be rebuilt with `npm run content:build`.
