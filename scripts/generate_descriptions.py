...
def generate(client, titles, max_retries=2):
    ...
    for attempt in range(max_retries + 1):
        ...
        if len(text) <= 160:
            return text
        # Feed the over-long answer back and ask it to trim.
        messages = messages + [
            {"role": "assistant", "content": text},
            {"role": "user", "content": RETRY_PROMPT.format(
                over=len(text) - 160, length=len(text))},
        ]
    return text  # return last attempt even if still over
