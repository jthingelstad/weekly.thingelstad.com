...
TEMPLATE = REPO / "aws" / "cloudformation.yaml"
API_DIR = REPO / "librarian_api"
STREAM_DIR = REPO / "librarian_stream"
BUILD_DIR = REPO / "tmp" / "librarian_lambda"
STREAM_BUILD_DIR = REPO / "tmp" / "librarian_stream_lambda"
ZIP_PATH = REPO / "tmp" / "librarian_lambda.zip"
STREAM_ZIP_PATH = REPO / "tmp" / "librarian_stream_lambda.zip"
...
def tinylytics_api_site_id() -> str:
    explicit = os.environ.get("TINYLYTICS_API_SITE_ID") or os.environ.get("LIBRARIAN_TINYLYTICS_SITE_ID")
    if explicit:
        return explicit
    configured = os.environ.get("TINYLYTICS_SITE_ID", "")
    return configured if configured.isdigit() else DEFAULT_TINYLYTICS_API_SITE_ID
...
def package_lambda() -> None:
    ...
    if requirements.exists():
        run([sys.executable, "-m", "pip", "install", "-r", str(requirements), "-t", str(BUILD_DIR)])
...
def package_stream_lambda() -> None:
    if STREAM_BUILD_DIR.exists():
        shutil.rmtree(STREAM_BUILD_DIR)
    STREAM_BUILD_DIR.mkdir(parents=True)

    for path in STREAM_DIR.iterdir():
        if path.is_file():
            shutil.copy2(path, STREAM_BUILD_DIR / path.name)

    if (STREAM_BUILD_DIR / "package-lock.json").exists():
        run(["npm", "ci", "--omit=dev", "--no-audit", "--no-fund"], cwd=STREAM_BUILD_DIR)
    else:
        run(["npm", "install", "--omit=dev", "--no-audit", "--no-fund"], cwd=STREAM_BUILD_DIR)

    STREAM_ZIP_PATH.parent.mkdir(parents=True, exist_ok=True)
    if STREAM_ZIP_PATH.exists():
        STREAM_ZIP_PATH.unlink()
    with zipfile.ZipFile(STREAM_ZIP_PATH, "w", zipfile.ZIP_DEFLATED) as archive:
        for path in STREAM_BUILD_DIR.rglob("*"):
            if path.is_file():
                archive.write(path, path.relative_to(STREAM_BUILD_DIR))
...
def main() -> int:
    ...
    parser.add_argument("--tinylytics-site-id", default=tinylytics_api_site_id())
